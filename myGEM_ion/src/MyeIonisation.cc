...
G4eIonisation::G4eIonisation(const G4String& name)
  : G4VEnergyLossProcess(name),
    ...
    isInitialised(false)
{
  G4ProductionCutsTable::GetProductionCutsTable()->SetEnergyRange(0.8*CLHEP::eV, 100*CLHEP::TeV);
		    ...
		    const G4ParticleDefinition*)
{
  if(!isInitialised) {
    ...
    EmModel(0)->SetLowEnergyLimit(0.8*CLHEP::eV);
